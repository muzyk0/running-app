...
`
	...
	result, err := NewNormalizer().Normalize(rawOutput, sampleGenerateRequest())
	if err != nil {
...
func TestNormalizerNormalizeRejectsUnsupportedStepType(t *testing.T) {
  ...
  "training": {
	...
	_, err := NewNormalizer().Normalize(rawOutput, sampleGenerateRequest())
	if err == nil {
...
func sampleGenerateRequest() provider.GenerateRequest {
